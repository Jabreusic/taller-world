...
const TRABAJOS_KEY = 'trabajosActivos';
...
function restaurarTrabajosActivos() {
    try {
        const raw = localStorage.getItem(TRABAJOS_KEY);
        if (!raw) return;
        ...
        window.reparacionesActivas = trabajos.map(t => {
            ...
            // Si terminó, marcar como listo para cobro o siguiente estado
            if (segundosPendientesReal === 0 && t.estado !== 'listoParaCobro') {
...
function procesarDevolucionDeudasMecanicosPorCasos(hitoCasoForzado) {
    ...
    (mecanicos || []).forEach(function(m) {
        ...
        var chancePago = Math.max(0.08, Math.min(0.88, 0.22 + ((m.lealtad || 0) * 0.07)));
...
function obtenerBioMecanicoSeguro(nombre) {
    ...
    const base = biografiasMecanicos[nombre] || {};
    return {
...
const UI_EDITOR_KEY = 'tw_ui_editor_v1';
...
function construirAccionesEventoBanco(tipoEvento) {
    ...
    if (tipoEvento === 'muy_endeudado') {
        return [
            { texto: 'Renegociar deuda', accion: 'renegociar_deuda' },
            { texto: 'Aceptar penalizacion', accion: 'aceptar_penalizacion', clase: 'btn-danger' },
            { texto: 'Pedir rescate financiero', accion: 'pedir_rescate_financiero' }
        ];
    }
    return [
...
function abrirModal(tipo) {
    ...
    if (tipo === 'inversiones') {
        ...
        // Eliminada UI de historia por día
...
function obtenerProgresoOperativoActual() {
    ...
    const casos = Math.max(0, Math.round(clientesHoy || 0));
...
function renderizarTiendasMejoras() {
    ...
    const casosCerrados = Math.max(0, Math.round(clientesHoy || 0));
...
function cerrarModal() {
    if (pantallaActiva === 'oficina' || pantallaActiva === 'configuracion') lugarActual = 'Oficina';
    else if (pantallaActiva === 'exterior' || pantallaActiva === 'mapa') lugarActual = 'Exterior';
    ...
    const mecanicosHtml = mecanicos.map(function(m, idx) {
        ...
        return `<div class="equipo-of-card" id="equipo-of-card-${idx}">
            ...
            <div class="equipo-of-bars">
                ...
                <div class="equipo-of-bar-row">
                    <span>Lealtad</span>
                    <div class="equipo-of-track"><div class="equipo-of-fill loyalty" style="width:${Math.round(m.lealtad * 10)}%"></div></div>
                    <span>${m.lealtad}/10</span>
                </div>` : ''}
            </div>
...
function hablarConMecanicoDesdeOficina(idx) {
    ...
    if (typeof mostrarFeedbackGameplay === 'function') mostrarFeedbackGameplay(`${m.nombre}: "${frase}"`, 'ok');
    renderizarEquipoOficina();
}
...
function cobrarDeudaMecanicoOficina(idx) {
    ...
    const exito = Math.random() < (0.3 + (m.lealtad || 0) * 0.07);
    if (exito) {
...
function requisitosMejora(tipo) {
    const base = (ECONOMY_DATA.requisitosMejora || {})[tipo];
...
function comprarMejora(tipo) {
    const progresoOperativo = obtenerProgresoOperativoActual();
    const req = requisitosMejora(tipo);
    const nivelActual = tipo === 'maquina' ? (mejoras.maquinaDiagnosis ? 1 : 0) : mejoras[tipo];
...
const forzarNueva = sessionStorage.getItem('tw_force_new') === '1';
const restauradaAutomaticamente = !forzarNueva && (typeof cargarPartidaSilenciosaInicial === 'function')
    ? cargarPartidaSilenciosaInicial()
    : false;
if (forzarNueva) {
    sessionStorage.removeItem('tw_force_new');
    var menuInicioEl = typeof obtenerMenuInicioEl === 'function'
        ? obtenerMenuInicioEl()
    ...
    // DEPURACIÓN: Mensaje visible para saber si el inicio se ejecuta
    let debugDiv = document.createElement('div');
    debugDiv.id = 'debug-inicio-juego';
    ...
    debugDiv.style.fontSize = '1.2em';
    debugDiv.style.borderRadius = '8px';
    debugDiv.innerText = 'INICIO DE JUEGO: función de inicio ejecutada';
    document.body.appendChild(debugDiv);
    setTimeout(function(){
      if(document.getElementById('debug-inicio-juego'))
        document.getElementById('debug-inicio-juego').remove();
    }, 3500);
} else if (restauradaAutomaticamente) {
    var menuInicioRestauradoEl = typeof obtenerMenuInicioEl === 'function'
        ? obtenerMenuInicioEl()
        : document.getElementById('menu-inicio');
    if (menuInicioRestauradoEl) menuInicioRestauradoEl.classList.add('hidden');
    document.getElementById('game').classList.remove('hidden');
} else {
    document.getElementById('game').classList.add('hidden');
}
...
function prepararEntornoBalanceTaller(paralelos) {
    ...
    if (typeof asegurarRitmoTaller === 'function') asegurarRitmoTaller();
    if (typeof obtenerPendientesNarrativaTelefono === 'function') {
...
window.testBalanceTaller = async function(totalCasos, opciones) {
    ...
    prepararEntornoBalanceTaller(paralelos);
